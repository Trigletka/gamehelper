...
def handle_text(message):
    ...
    if message.text.lower() == 'сменить раздел':
        current_section = None
    elif current_section is None:
        bot.send_message(message.chat.id, 'Выберите раздел с помощью команды "сменить раздел"')
...
def next_month(call):
    ...
    bot.send_message(call.message.chat.id, "Календарь выхода игр на " + monthes[localtime()[1]+c - 1] + ':\n' + '\n'.join(x),
                     reply_markup=keyboard_calendar)
...
def find_game(message):
    ...
    if message.text.lower() == '/search':
        if current_section == 'some_section':
